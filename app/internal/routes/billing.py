...
async def add_money(user_id: int, amount: float, transaction_type: TRANSACTION_TYPE_CHOICES_ENUM, db: AsyncSession = Depends(get_session)):
...
async def debit_money(user_id: int, amount: float, transaction_type: TRANSACTION_TYPE_CHOICES_ENUM, db: AsyncSession = Depends(get_session)):
